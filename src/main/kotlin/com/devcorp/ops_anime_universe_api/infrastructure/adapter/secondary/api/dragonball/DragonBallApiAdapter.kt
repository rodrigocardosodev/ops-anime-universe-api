...
class DragonBallApiAdapter(
        ...
        override suspend fun getCharacters(
                ...
                // Caso padrão para qualquer página, inclusive página 0
                ...
                return withContext(Dispatchers.IO) {
                        try {
                                ...
                                logger.error(
                                        "Erro ao buscar personagens do Dragon Ball: ${e.message}",
                                        e
                                )
                                createEmptyResponse(dragonBallPage, validLimit)
                        }
                        ...
                        try {
                                val response =
                                        dragonBallWebClient
                                                ...
                                                .awaitBodilessEntity()
                                logger.info(
                                        "Status code da Dragon Ball API: {}",
                                        response.statusCode
                                )
                                response.statusCode == HttpStatus.OK
                        } catch (e: Exception) {
                                logger.error(
                                        "Erro ao verificar disponibilidade da API do Dragon Ball: ${e.message}",
                                        e
                                )
                                false
                        }
